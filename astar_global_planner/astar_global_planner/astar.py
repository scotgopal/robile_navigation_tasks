...
class AStarGlobalPlanner(Node):
    ...
    * publishes the resulting ``/plan`` (Path)
    ...
    def _goal_pose_cb(self, msg: PoseStamped):
        ...
        path_msg = self.compute_path(start_world, goal_world, goal_orientation=goal_orientation)
    ...
    def compute_path(
        ...
        self._publish_waypoints(path_world, goal_orientation=goal_orientation)

        # Build Path msg from the full dense path
        path_msg = self._build_path_msg(full_path, goal_orientation=goal_orientation)
        return path_msg

    # ------------------------------------------------------------------
    # Helpers
    # ------------------------------------------------------------------
    def _publish_waypoints(self, path_world: List[Tuple[float, float]], goal_orientation=None):
        ...
        # Override the last waypoint orientation with the goal orientation
        if goal_orientation is not None and len(pose_array.poses) > 0:
            pose_array.poses[-1].orientation = goal_orientation
    ...
    def _build_path_msg(
        self, path_world: List[Tuple[float, float]], goal_orientation=None
    ) -> Path:
        msg = Path()
        msg.header = Header()
        ...
        # Override the last pose orientation with the goal orientation
        if goal_orientation is not None and len(msg.poses) > 0:
            msg.poses[-1].pose.orientation = goal_orientation
