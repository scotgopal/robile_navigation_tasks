...
class GridCostmap:
    ...
    def __init__(self):
        ...
        self.data: list = []
        self.raw_data: list = []  # un-inflated copy for start/goal checks
    ...
    def update_from_occupancy_grid(self, msg: OccupancyGrid):
        ...
        self.raw_data = list(msg.data)  # keep a copy before inflation
    ...
    def is_free(self, mx: int, my: int) -> bool:
        """Return True when the cell is traversable (inflated costmap)."""
    ...
    def is_free_raw(self, mx: int, my: int) -> bool:
        """Return True when the cell is traversable in the RAW (un-inflated) map.
        
        Used for validating start position — the robot IS there, so
        if the raw map says it's free, we should plan from there even
        if the inflated costmap marks it as occupied.
        """
        if not self.in_bounds(mx, my):
            return False
        idx = my * self.width + mx
    ...
    def clamp_to_bounds(self, mx: int, my: int) -> Tuple[int, int]:
        """Clamp map coordinates to valid grid bounds."""
        mx = max(0, min(self.width - 1, mx))
        my = max(0, min(self.height - 1, my))
        return mx, my

    def find_nearest_free_cell(
        self, mx: int, my: int, max_radius: int = 100,
        use_raw: bool = False,
    ) -> Optional[Tuple[int, int]]:
        """
        BFS outward from (mx, my) to find the nearest free cell.

        If (mx, my) is out of bounds, first clamps to the boundary
        and searches from there.

        Parameters
        ----------
        use_raw : bool
            When True, use the un-inflated (raw) map for the free check.
            This is used for finding start cells so the result is
            compatible with astar_search's is_free_raw validation.
            The found cell must also have at least one free neighbor
            to ensure A* can expand from it.
        """
        # Clamp the starting point to within bounds
        sx, sy = self.clamp_to_bounds(mx, my)

        check_fn = self.is_free_raw if use_raw else self.is_free

        def _has_free_neighbor(cx: int, cy: int) -> bool:
            """Return True if at least one neighbor passes check_fn."""
            for ddx in (-1, 0, 1):
                for ddy in (-1, 0, 1):
                    if ddx == 0 and ddy == 0:
                        continue
                    if check_fn(cx + ddx, cy + ddy):
                        return True
            return False

        # If the clamped cell is already free and has connectivity, return it
        if check_fn(sx, sy) and (not use_raw or _has_free_neighbor(sx, sy)):
            return (sx, sy)

        # BFS to find the nearest free cell with connectivity
        queue: deque = deque()
            ...
            for ddx in (-1, 0, 1):
                for ddy in (-1, 0, 1):
                    ...
                    if not self.in_bounds(nx, ny):
                        continue
                    visited.add((nx, ny))
                    if check_fn(nx, ny) and (not use_raw or _has_free_neighbor(nx, ny)):
                        return (nx, ny)
...
def astar_search(
    ...
    # Allow start in inflation zone (robot IS there) — use raw map
    if not grid.is_free_raw(*start):
        return None
    ...
    while open_set:
        ...
        for dx, dy, move_cost in _MOVEMENTS:
            ...
            # Allow cells near start that are free in raw map (inflation zone)
            is_near_start = (abs(cx - start[0]) <= 2 and abs(cy - start[1]) <= 2)
            if is_near_start:
                cell_ok = grid.is_free_raw(nx, ny)
            else:
                cell_ok = grid.is_free(nx, ny)

            if not cell_ok or neighbor in closed_set:
                continue
...
def _perpendicular_distance(
    ...
    # Project point onto the line (unclamped parameter t)
...
def douglas_peucker(
    ...
    line between their neighbours is less than *epsilon* (metres).
    Start and goal are always kept.

    Parameters
    ----------
    path : list of (x, y)
        Input waypoints in world coordinates.
    epsilon : float
        Distance threshold in metres.  Smaller values keep more detail;
        larger values produce fewer waypoints.
    min_points : int
        Minimum number of waypoints to retain (including start & goal).
    """
    if len(path) <= 2:
        return list(path)

    # Iterative stack-based implementation (avoids recursion limit)
    keep = [False] * len(path)
    keep[0] = True
    keep[-1] = True
    ...
    while stack:
        ...
        for i in range(start_idx + 1, end_idx):
            d = _perpendicular_distance(path[i], path[start_idx], path[end_idx])
            if d > max_dist:
                max_dist = d
                farthest = i
    ...
    simplified = [pt for pt, k in zip(path, keep) if k]

    # Guarantee a minimum number of waypoints
    if len(simplified) < min_points and len(path) >= min_points:
        # Fall back to uniform sub-sampling
        step = max(1, (len(path) - 1) // (min_points - 1))
...
class AStarGlobalPlanner(Node):
    ...
    * publishes the resulting ``/plan`` (Path)
    ...
    def __init__(self):
        ...
        self.declare_parameter('do_simplify_path', True)
        self.declare_parameter('simplify_epsilon', 0.15)  # metres – Douglas-Peucker tolerance
        self.declare_parameter('simplify_min_points', 3)   # minimum waypoints to keep
        ...
        self._do_simplify = self.get_parameter('do_simplify_path').value
        self._simplify_epsilon = self.get_parameter('simplify_epsilon').value
        self._simplify_min_points = self.get_parameter('simplify_min_points').value
        ...
        self.waypoints_array_pub = self.create_publisher(PoseArray, '/waypoints_array', 10)
    ...
    def compute_path(
        ...
        # --- Handle out-of-bounds or non-free start/goal -----------------
        # For start: use raw (un-inflated) map since the robot IS there
        if not self.grid.is_free_raw(*start_map):
            self.get_logger().warn(
                f'Start {start_map} is out of bounds or occupied in raw map, '
                f'searching for nearest free cell...'
            )
            nearest = self.grid.find_nearest_free_cell(*start_map, use_raw=True)
            if nearest is None:
                self.get_logger().error(
                    'No free cell near start – cannot plan'
                )
                return None
            self.get_logger().info(
                f'Using nearest free cell {nearest} as start '
                f'(was {start_map})'
            )
            start_map = nearest
        elif not self.grid.is_free(*start_map):
            # Start is free in raw map but in inflated zone — that's OK,
            # the robot is there. Log but allow it.
            self.get_logger().info(
                f'Start {start_map} is in inflation zone but free in raw '
                f'map — using it directly'
            )

        if not self.grid.is_free(*goal_map):
            self.get_logger().warn(
                f'Goal {goal_map} is out of bounds or not free, '
                f'searching for nearest free cell...'
            )
            nearest = self.grid.find_nearest_free_cell(*goal_map)
            if nearest is None:
                self.get_logger().error(
                    'No free cell near goal – cannot plan'
                )
                return None
            self.get_logger().info(
                f'Using nearest free cell {nearest} as goal '
                f'(was {goal_map})'
            )
            goal_map = nearest
        ...
        # Save the full dense path (smoothed but not simplified) for /plan
        full_path = list(path_world)

        # Douglas-Peucker simplification → key waypoints only
        if self._do_simplify and len(path_world) > 2:
            before = len(path_world)
            path_world = douglas_peucker(
                path_world,
                epsilon=self._simplify_epsilon,
                min_points=self._simplify_min_points,
            )
            self.get_logger().info(
                f'Douglas-Peucker: {before} → {len(path_world)} waypoints '
                f'(ε={self._simplify_epsilon:.3f} m)'
            )

        # Publish simplified waypoints on /waypoints and /waypoints_array
        self._publish_waypoints(path_world)

        # Build Path msg from the full dense path
        path_msg = self._build_path_msg(full_path)
        return path_msg

    # ------------------------------------------------------------------
    # Helpers
    # ------------------------------------------------------------------
    def _publish_waypoints(self, path_world: List[Tuple[float, float]]):
        """Publish waypoints individually on /waypoints and as a PoseArray on /waypoints_array."""
        stamp = self.get_clock().now().to_msg()
        prev_x, prev_y = path_world[0]

        # PoseArray for visualising all waypoints at once
        ...
        for x, y in path_world:
            ...
            # Individual PoseStamped on /waypoints
            wp = PoseStamped()
            wp.header.stamp = stamp
            wp.header.frame_id = self.grid.frame_id
            wp.pose = pose
        ...
        self.get_logger().info(
            f'Published {len(path_world)} waypoints to /waypoints and /waypoints_array'
        )
